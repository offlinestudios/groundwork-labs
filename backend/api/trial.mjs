...
function supportHtml(lead) {
  const fields = [
    ...
    ['Contact', lead.contactName || 'Not supplied'],
    ['Email', lead.email],
...
function customerHtml(lead) {
  const name = htmlEscape(firstName(lead.contactName || lead.businessName));
...
export default async function handler(request, response) {
  ...
  if (!lead.contactName || !lead.businessName || !isValidEmail(lead.email)) {
    return response.status(400).json({ ok: false, error: 'Please provide your name, business name, and a valid email address.' });
  }
  ...
  try {
    ...
    if (existing.length) {
      ...
      await sql`
        INSERT INTO leads (
          id, submission_id, business_name, contact_email, contact_name, industry, business_website, monthly_volume,
          funnel, landing_path, referrer, utm_source, utm_medium, utm_campaign,
          utm_term, utm_content, fbclid, status, email_status
        ) VALUES (
          ${leadId}, ${submissionId}, ${lead.businessName}, ${lead.email}, ${lead.contactName || null}, ${lead.industry || null}, ${lead.businessWebsite || null}, ${lead.monthlyVolume || null},
